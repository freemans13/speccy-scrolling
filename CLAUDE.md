...
- **Dither by push-order** — the A/B checkerboard column shading in `PIPE_PROGRAM` is achieved purely by alternating the push order per row (A-rows `push de`, B-rows `push hl`), with zero per-row cost. (An earlier EXX register-bank-swap dither was removed — see the EXX/djnz bank-confusion pitfall in auto-memory.)
...
- **`PIPE_PROGRAM`** at `$DB00..$F400` — machine-emitted SMC slot grid, `NUM_BANDS=60` bands (20 char-cells × 3 pipes) of `BAND_STRIDE=80` bytes. Each band = `ld ix,screenbase` (+0) then 8 six-byte body rows (+4..+51); the band trailer `JP next_band_base` sits at `+68` (so +52..+67 is a NOP bridge the body chain falls through). A body row is `ld sp,ix ; push de`(A-row)/`push hl`(B-row) `; push bc ; inc ixh` — the A/B checkerboard dither is by **push order**, NOT EXX. Cap rows are `JP cap_handler`. Epilogue at `SLOT_GRID_END` (= base + 60·80) is `ld sp,(saved_sp); ret`. Scrolling patches only each band's `ld ix` operand (via `patch_pipe_targets`).
- **Self-recycle** — there is ONE grid (no GRID_A/GRID_B double-buffer, no prep-column leapfrog — those were removed; this is the `patch-only-renderer` branch). When a pipe reaches `byte_x==1`, `do_swap` JR-skips its departing column (`write_jrskip_column`) and rebuilds it in place at the new gap_y; `do_swap_part_b` finishes the rebuild on the following frame (`activate_pipe_idx`). (`prep_step`, `clear_vacated_columns`, `clear_pending` no longer exist.)
